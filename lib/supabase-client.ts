...
// 클라이언트 사이드용 Supabase 클라이언트 (기본)
export const supabase = createClient<Database>(supabaseUrl, supabaseAnonKey)

// 클라이언트 사이드용 Supabase 클라이언트 (SSR 지원)
...
export const checkUsageLimit = async (userId: string) => {
  const client = createClientSupabase()
  const { data, error } = await client.rpc('check_user_usage_limit', {
    user_uuid: userId
  })
...
export const incrementUsage = async (userId: string) => {
  const client = createClientSupabase()
  const { error } = await client.rpc('increment_user_usage', {
    user_uuid: userId
  })
